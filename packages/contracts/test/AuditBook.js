...
describe('AuditBookNFT', function () {
    ...
    describe('Mint', function () {
        it('Should success', async function () {
            const { auditBook, testERC20, owner, otherAccount } =
                await loadFixture(deployAuditBookFixture);
            await expect(testERC20.approve(auditBook.address, initialPrice)).not
                .to.be.reverted;
    ...
    describe('TokenURI', function () {
        describe('setTokenURI', function () {
            it('Should success by owner', async function () {
                ...
                await expect(auditBook.setTokenURI('ipfs://newTokenUri')).not.to
                    .be.reverted;
                expect(await auditBook.tokenURI(0)).to.equal(
                    'ipfs://newTokenUri'
                );
            });
    ...
    describe('chai', function () {
        describe('setPrice', function () {
            ...
            it('Should success by owner', async function () {
                ...
                await expect(testERC20.approve(auditBook.address, initialPrice))
                    .not.to.be.reverted;
                await expect(
                    auditBook.safeMint(owner.address)
            ...
            it('Should fail by otherAccount', async function () {
                ...
                await expect(
                    auditBook
                        .connect(otherAccount)
                        .safeMint(otherAccount.address)
                ).not.to.be.reverted;
                expect(
                    await auditBook.balanceOf(otherAccount.address)
                ).to.equal(1);
                expect(await auditBook.ownerOf(0)).to.equal(
                    otherAccount.address
                );
            });
        });
